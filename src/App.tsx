...
const App = () => {
  ...
  const handleManualSave = async (containerNo, amount) => {
    ...
    await setDoc(doc(db, "orders", orderId), {
      ...
      '數量': 1, '單價': Number(amount), '品名': 'Direct Shipment'
    });
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (isSimpleChina) {
          ...
          headers.forEach((header, index) => {
            ...
            if (header === '數量') {
              rowData['數量明細'] = val; // 保留 "40*30R+15=1215Y" 以供印單
              let parsedQty = 0;
              if (val.includes('=')) {
                // 如果有等號，抓取等號後面的數字 (例如 =1215Y 會抓出 1215)
                parsedQty = parseFloat(val.split('=')[1].replace(/[^0-9.-]/g, '')) || 0;
              } else {
                parsedQty = parseFloat(val.replace(/[^0-9.-]/g, '')) || 0;
              }
              rowData['數量'] = parsedQty;
            } else {
              rowData[header] = val;
            }
          });
  ...
  const handleAiAnalysis = async () => { /* AI Logic Maintained */ };
      ...
      <nav className="bg-[#0F172A] text-white px-6 py-4 shadow-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold tracking-wide">EVERISE <span className="text-emerald-400 text-xs px-2 border rounded-full">V20</span></h1>
          </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {masterData.filter(i => Object.values(i).some(v => String(v).toLowerCase().includes(searchTerm.toLowerCase()))).map(row => (
                      <tr key={row.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-3">{isChinaOrder(row) ? <span className="text-red-600 bg-red-50 px-2 py-1 rounded">China</span> : 'Warehouse'}</td>
